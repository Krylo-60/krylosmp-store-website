...
document.addEventListener('DOMContentLoaded', () => {
  const btnSupportChat = document.getElementById('btnSupportChat');
  const supportChatWindow = document.getElementById('supportChatWindow');
  const btnCloseChat = document.getElementById('btnCloseChat');
  const btnStartChatThread = document.getElementById('btnStartChatThread');
  const chatHomeTab = document.getElementById('chatHomeTab');
  const chatMessagesTab = document.getElementById('chatMessagesTab');
  ...
  const lblChatHeaderGreeting = document.getElementById('lblChatHeaderGreeting');

  if (!btnSupportChat) return;

  // Toggle Chat Window
  btnSupportChat.addEventListener('click', () => {
    const isVisible = supportChatWindow.style.display === 'flex';
    supportChatWindow.style.display = isVisible ? 'none' : 'flex';
    
    // Update Greeting Username
    if (state.mcUsername) {
      lblChatHeaderGreeting.textContent = `Hi ${state.mcUsername} 👋`;
    } else {
      lblChatHeaderGreeting.textContent = 'Hi there 👋';
    }
  });
  ...
  // Switch to messages on thread start
  btnStartChatThread.addEventListener('click', () => {
    chatHomeTab.style.display = 'none';
    chatMessagesTab.style.display = 'flex';
    tabHomeBtn.style.color = 'var(--text-muted)';
    tabMessagesBtn.style.color = '#3742fa';
    chatInput.focus();
  });

  btnBackToHome.addEventListener('click', () => {
    chatMessagesTab.style.display = 'none';
    chatHomeTab.style.display = 'flex';
    tabHomeBtn.style.color = '#3742fa';
    tabMessagesBtn.style.color = 'var(--text-muted)';
  });
  ...
  btnSendChat.addEventListener('click', triggerSendMessage);
  ...
  function appendMessage(role, text) {
    ...
    row.style.alignSelf = isBot ? 'flex-start' : 'flex-end';
    row.style.maxWidth = '85%';
    row.style.background = isBot ? 'rgba(255,255,255,0.05)' : 'linear-gradient(135deg, #5352ed, #3742fa)';
    row.style.border = isBot ? '1px solid var(--glass-border)' : 'none';
    row.style.borderRadius = isBot ? '12px 12px 12px 2px' : '12px 12px 2px 12px';
    ...
    row.innerHTML = text
      ...
      .replace(/\*(.*?)\*/g, '<em>$1</em>')
      .replace(/`(.*?)`/g, '<code style="background:rgba(0,0,0,0.2);padding:0.1rem 0.3rem;border-radius:4px;">$1</code>');
  ...
  function appendTypingIndicator() {
    ...
    row.style.borderRadius = '12px 12px 12px 2px';
    row.style.padding = '0.8rem 1rem';
    row.style.fontSize = '0.85rem';
    row.style.color = 'var(--text-muted)';
    row.style.marginTop = '0.4rem';
    row.innerHTML = `<span class="typing-dot" style="display:inline-block;width:6px;height:6px;background:#fff;border-radius:50%;margin-right:3px;animation:typing 1s infinite alternate;"></span><span class="typing-dot" style="display:inline-block;width:6px;height:6px;background:#fff;border-radius:50%;margin-right:3px;animation:typing 1s infinite alternate 0.2s;"></span><span class="typing-dot" style="display:inline-block;width:6px;height:6px;background:#fff;border-radius:50%;animation:typing 1s infinite alternate 0.4s;"></span>`;
    
    // Inject animation CSS if not present
    if (!document.getElementById('typingStyle')) {
      const style = document.createElement('style');
      style.id = 'typingStyle';
      style.innerHTML = `@keyframes typing { from { opacity: 0.2; transform: translateY(0); } to { opacity: 1; transform: translateY(-3px); } }`;
  ...
  async function sendChatMessage(userText) {
    ...
    try {
      const chatRes = await fetch('https://krims-code-chatbot.vercel.app/api/chat', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          prompt: userText,
          model: 'gemini',
          systemInstruction: `You are the Krims Code AI Store Assistant. Helpful, friendly, and expert in the KryloSMP Minecraft server.
Ranks: VIP (500 KC), MVP (1000 KC), Legend (2500 KC), Titan (5000 KC), Champion (7500 KC), Elite (10000 KC), Overlord (15000 KC), God (25000 KC), Immortal (50000 KC), Antigravity (100000 KC).
Keys: Season, Mythic, Legendary crate keys.
There is a 3% Game Tax on all regular store checkouts.
There is a 16% off discount on the 'Krylo Ultimate Bundle (Buy All)' which includes all 100 products for 226,983 KC (with a 5% game tax).
All promo codes (e.g. KRYLO, KRYLOSMP, WELCOMESMP) give a 20% discount and are restricted to 1 claim per IP. VPNs are blocked.`,
      ...
      if (chatRes.ok) {
        ...
        const responseText = chatData.text || chatData.response || "No response received.";
        ...
        appendMessage('bot', "Sorry, I am having trouble connecting to the support servers right now. Please try again later!");
      }
    } catch (err) {
      removeTypingIndicator();
      appendMessage('bot', "Connection error. Please check your internet connection!");
    }
  }
});
